...
const allMovies = [
    ...
    // Función para crear una tarjeta de película (MODIFICADA para el enlace)
    function createMovieCard(movie) {
        const movieCard = document.createElement('div');
        movieCard.classList.add('movie-card');

        // Envuelve la tarjeta de película en un enlace a la página de detalle
        const movieLink = document.createElement('a');
        movieLink.href = `detail.html?id=${movie.id}`; // Pasa el ID de la película como parámetro de URL

        const posterImg = document.createElement('img');
        posterImg.src = movie.poster;
        posterImg.alt = movie.title;
        posterImg.onerror = function() {
            this.onerror=null; // Evita bucle infinito en caso de error persistente
            this.src='https://via.placeholder.com/180x270?text=No+Image'; // Imagen de placeholder
        };
        ...
        movieCard.appendChild(movieLink); // Añade el enlace a la tarjeta
    ...
    function displayMovies(moviesToDisplay, page) {
        moviesContainer.innerHTML = ''; // Limpia el contenedor actual
        ...
        updateView(); // Aplica la vista actual (grid/list)
    ...
    function searchMovies() {
        ...
        currentPage = 1; // Resetear a la primera página para nuevas búsquedas
        displayMovies(filteredMovies, currentPage);
    }
    ...
    function updateView() {
        if (currentView === 'grid') {
            ...
            moviesContainer.classList.remove('movies-list');
            gridViewBtn.classList.add('active');
            listViewBtn.classList.remove('active');
        } else {
            moviesContainer.classList.add('movies-list');
            moviesContainer.classList.remove('movies-grid');
            listViewBtn.classList.add('active');
            gridViewBtn.classList.remove('active');
        }
    }

    // Manejadores de eventos para cambiar la vista
    ...
    // Leer parámetro de búsqueda de la URL si existe (para búsquedas desde la página de detalle)
